...
The ANOVA or **an**alysis **o**f **va**riance is a lot like a t-test that is used to compare more that two groups or categories with each other, conceptually.
...
After that, we can dig deeper with pairwise comparisons (so-called _post-hoc tests_) that adjust the alpha rate for several comparisons.
...
**2. Define the model ** 
    - Sum of Squares: Sum of Squares total, within & between (_R does this for us_)
...
`r mcq(c("The data are heterogeneous", answer = "The assumption is met, the data are homkogeneous", "p < .05 is always preferable in any test ever"))`

`r hide("More Info")`

Like in any other test, the logic here is that p < .05 would indicate significant _differences_. 
Since in this test we would rather find homogeneity, i.e. a lack of differences in variances, our assumption is met if p > .05.

`r unhide()`
    
#### 2. Define the overall model {-}

In R, the basic function for calculating an ANOVA model is `aov()`, where we use formula notation (so using the tilde ~) to describe `aov(dependent variable ~on the basis of~ categorial variable, data)`.
Commonly, we then examine the results using the `summary()` call.
...
`r mcq(c("The result is significant", answer = "The result is non-significant", "Different music genres predict loudness of the music.", "We definitely need to calculate post-hoc tests now to find out where there may be differences."))`
    
If an omnibus (_overall_) ANOVA turns out to be significant, we must conduct further tests to find out where there are pairwise differences.
...
The most common post-hoc test to use in R is the Tukey Honest Significant Differences `TukeyHSD()` test. 
...
`r mcq(c("There is a significant pairwise difference between rock and pop.", "There is a trend for the difference in volume between techno and rock.", answer = "There are no significant pairwise differences in our (small) sample."))`
...
**Bootstrapping** is a more advanced technique for taking random resamples of the original data and basically pretending like we had a larger sample.
