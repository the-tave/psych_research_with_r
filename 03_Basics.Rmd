...
`x <- c(1, 2, 3)`
    - To avoid a cluttered work environment, `rm()` removes variables
...
- `Library(greatpackage)` - two problems:
    - `library` should not be capitalized
    - "greatpackage" does not exist and can thus not be loaded
- `mean(coolvariable)`: coolvariable was not defined previously
- `a <- c(1, 3, 6, 7`: the command is not finished, it needs a closing parentheses
- `b <- c(2, 4; 6, 8)`: we need all commas to seperate numbers in a vector, not a semicolon
