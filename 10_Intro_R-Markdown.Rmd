...
> <i class="fa-solid fa-lightbulb" style="color: purple;"></i> <i style="color: purple;"> The YAML header needs to be indented in a certain way, otheriwse the commands are not recognized! </i>
...
> <i class="fa-solid fa-lightbulb" style="color: purple;"></i> <i style="color: purple;"> In the YAML header, `true` and `false` are not capitalized, unlike in the rest of R! </i>
...
It is also possible to use inline Code to report statistics, although it may be more efficient for reporting to create tables with `kable`, which you can read more about [here](https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_html.html).
...
![Shakespeare writing RMarkdown (AI generated using the [bing image creator](https://www.bing.com/images/create))](img/shakespeare.jpg){height=550px}